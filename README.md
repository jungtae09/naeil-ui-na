...
> 오늘의 행동 → 작은 성취 → 꾸준함 → 이번 주 목표 → 더 나은 내일
...
localStorage는 **테마·알림 같은 기기별 설정에만** 쓰입니다.
...
│   ├── StampGrid         10칸 도장판
│   ├── ProgressBar       진행률
│   ├── QuoteCard         오늘의 한마디 / 오늘의 질문 + 한 줄 답
...
│   ├── Today             ★ 메인 화면
│   ├── History           통계 · 달력 · 배지 (조회 전용)
│   ├── Friends           친구 코드 · 요청 · 순위 · 응원
│   ├── Weekly            주간 분석 + 이번 주 목표 한 줄
...
│   ├── profiles / rules / records / friends / notes
│   └── stats             연속 기록 · 달성률 계산
│
├── utils/
...
**이번 주 목표는 한 줄**
긴 회고를 쓰게 하면 아무도 안 씁니다. "하루에 1km 걷기" 처럼 한 줄만 적으면
오늘 화면 맨 위에 계속 보입니다. (`weekly_reviews.next_goal` 재사용)

**기록은 오늘 것만 고칠 수 있다**
지난 날짜는 조회만 됩니다. 하루 안에 돌아보는 리듬을 지키기 위해 일부러 그렇게 뒀습니다.
...
- 다음 주 목표 자동 추천 (지금은 직접 한 줄로 적는 방식)
- 친구들 주간 결산
