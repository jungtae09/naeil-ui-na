...
데이터는 전부 Supabase에 저장됩니다. localStorage는 **테마 설정에만** 쓰입니다.
...
│   ├── Landing           첫 화면
│   ├── Login / Signup    인증
│   ├── Onboarding        3단계 안내 → 프로필 → 10가지 → 그룹
│   ├── Today             ★ 메인 화면
│   ├── History           통계 · 달력 · 배지
│   ├── Friends           그룹 · 순위 · 응원
│   ├── Weekly            주간 분석
│   ├── Settings          프로필 · 테마 · 그룹 · 로그아웃
...
**프라이버시**
그룹원에게 나가는 값은 `group_member_stats` RPC가 서버에서 계산한 것뿐입니다:
...
**사용자 A**
회원가입 → 닉네임 설정 → 10가지 작성 → 그룹 만들기 → 초대 코드 확인 → 오늘 체크 → 로그아웃

**사용자 B**
회원가입 → 초대 코드 입력 → 참여 → 자신의 10가지 작성 → 오늘 체크
...
- [ ] 친구의 **규칙 내용**은 보이지 않는다
- [ ] 4명까지 참여되고, 5번째는 "정원이 찼습니다" 안내가 나온다
- [ ] 잘못된 초대 코드 → "존재하지 않는 초대 코드입니다"
...
- 그룹 주간 결산
- Supabase Realtime — 친구가 체크하면 즉시 반영
- 저녁 알림
