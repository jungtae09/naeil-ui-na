...
import { myCheerCount } from '../services/groups'
        ...
        <>
          ...
          {selected && (
            <section className="card px-5 py-5 animate-fadeUp">
              <h2 className="text-sm font-bold text-ink">{formatKorean(selected)}</h2>
              {detailLoading ? (
                <Skeleton className="mt-3 h-24 w-full" />
              ) : dayDetail?.records?.length ? (
                <>
                  <p className="mt-1 text-xs text-muted">
                    {dayDetail.records.length}개 완료
                    {statsByDate.get(selected)?.is_complete && ' · 완주'}
                  </p>
                ...
                <p className="mt-3 text-sm text-muted">이날은 기록이 없습니다.</p>
              )}
            </section>
          )}
